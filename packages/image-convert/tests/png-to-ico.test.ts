...
describe("convertToIco", () => {
  ...
  it("should handle edge cases for sizes", async () => {
    ...
    await expect(convertToIco(pngBuffer, { sizes: [1] })).rejects.toThrow(
      "Size 1 is out of range. Must be between 16 and 256 pixels.",
    );

    // Test avec une taille très grande
    await expect(convertToIco(pngBuffer, { sizes: [1024] })).rejects.toThrow(
      "Size 1024 is out of range. Must be between 16 and 256 pixels.",
    );
  ...
  it("should handle invalid input", async () => {
    ...
    await expect(convertToIco(invalidBuffer)).rejects.toThrow();

    // Test avec un type invalide
    const invalidInput: any = "not a buffer";
    await expect(convertToIco(invalidInput)).rejects.toThrow(
